...
def save_data_to_csv(ticker, data, output_dir='data/japan_historical_data'):
    """
...
def update_progress_log(results, log_file='data/japan_historical_data_progress.csv'):
    """
    進捗状況をCSVファイルに保存する
...
def check_nasdaq_process():
    """
    NASDAQ銘柄の処理プロセスが実行中かどうかを確認する
    
    Returns:
    --------
    bool
        実行中の場合はTrue、そうでない場合はFalse
    """
    try:
        # psコマンドでプロセスを確認
        import subprocess
        ...
        # 出力を確認
        return 'fetch_nasdaq_historical_data.py' in result.stdout
    except Exception as e:
        print(f"プロセスの確認中にエラーが発生しました: {e}")
        traceback.print_exc()
        # エラーが発生した場合は安全のためTrueを返す
        return True
...
def main():
    try:
        ...
        parser.add_argument('--output-dir', type=str, default='data/japan_historical_data', help='出力ディレクトリ')
        ...
        parser.add_argument('--force', action='store_true', help='NASDAQ処理の実行中でも強制的に実行する')
        parser.add_argument('--skip-etf', action='store_true', help='ETF・ETNをスキップする')
        args = parser.parse_args()
        
        # NASDAQ銘柄の処理が実行中かどうかを確認
        if not args.force and check_nasdaq_process():
            print("NASDAQ銘柄の処理が実行中です。処理が完了するまで待機するか、--forceオプションを使用して強制的に実行してください。")
            return
        
        print("東証上場銘柄の過去株価データ取得")
        print("==============================")
        ...
        if args.resume and os.path.exists('data/japan_historical_data_progress.csv'):
            try:
                print("前回の進捗状況を読み込んでいます...")
                progress_df = pd.read_csv('data/japan_historical_data_progress.csv')
        ...
        print(f"処理対象の最初の5銘柄: {tickers_to_process[:5]}")
        ...
        print(f"進捗状況は data/japan_historical_data_progress.csv に保存されました。")
