...
- **近すぎる既存名も避ける（厳格・基準を緩めない）**: 既存のAI/テックサービスと**綴り・響きが近い**ものは生成段階で外す
  （例: `velder`→`velerin` / `Lnar`→`Lunar.dev` / `Reedmere`→`Reed.ai` / `Bolsena`→`Bolna` はNG）。
  - **「完全一致」でなくても、語頭・語幹・リズムが近いAIサービスが1つでもあれば除外**。
    "やや近い(△)"を「許容」して通してはいけない。**△は通さない=実質✕扱い**。
  - 理由: 近い音を残すと、後の本調査や市場投入で**大きな手戻り**になる。最初から弾くのが安い。
...
4. **「モノの名詞」の転用（中立〜好印象・AIで未使用が多い・有力）** ★並列で必ず使う
   - **AI/ITと無関係な“具体物”の名詞**で、元の意味が**ニュートラル〜ポジティブ**なものを、**そのまま or 少しだけ綴りを変えて**候補にする。
   - 具体物は (1)覚えやすい (2)悪い意味が無い (3)**AIスタートアップにまだ使われていない**ことが多い、の3拍子。
     例の発想: ランプ/照明器具、布・織物・編み・刺繍のパターン名、家具、陶器・器・茶器、道具・工具、結び目・ロープ、
     植物・ハーブ・花、果実、宝石・鉱物、楽器、建築部材、船具、文具、菓子、地名由来のモノ名 等。
     （例: ある種のランプや布の名前＝中立で、AIでは未使用。`Coniston`もこの発想に近い。）
   - **集めるコツ**: 「○○ types / ○○ patterns / named ○○」(lamp types, fabric weaves, knot names, pottery shapes…)で
     具体物の固有名・型名を列挙 → 読みやすく・中立〜好印象・短め(2〜3音節)のものを抜き、そのまま/微修正して候補化。
   - 元の意味が**否定的・きな臭い・きわどい**もの（武器・病気・蔑称由来等）は外す。
