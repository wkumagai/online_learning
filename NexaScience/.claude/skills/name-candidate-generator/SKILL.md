...
- **きれいな音(ユーフォニー)を意識**: 流音(l, m, n, r)や開いた母音を中心に、**口に出して心地よい響き**にする。
  濁音の連続・子音クラスタ・カタカナにすると読みにくい音写は却下（例: 「コンドゥヴァ」は読みにくい＝NG）。
...
- **近すぎる既存名も避ける**: 既存のAI/テックサービスと**綴り・響きが酷似**するものは生成段階で外す
  （例: `velder`→`velerin`(既存)と近い / `Lnar`→`Lunar.dev`と近い、はNGだった）。
  最終的な実在/商標確認は brand-name-availability に委ねるが、**明らかに似ている語は出さない**。
...
- **軽い実在チェック（推奨・追加）**: 最終候補を WebSearch で `"<候補>" AI` 程度だけ素引きし、
  **同分野(AI/SaaS/開発者ツール)に同名サービスが一発で出る明白な衝突は、出力前に外す**。
  深追いはしない（網羅的な商標/ドメイン確認はBの段1へ）。これで「大半が衝突」を避ける。
